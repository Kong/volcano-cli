...
func New(deps cliruntime.Deps) *cobra.Command {
	...
	group := &cobra.Command{
		...
		Long: "docs fetches the Volcano documentation corpus from its source " +
			"(default: the Kong/volcano-docs repository's /content on main; the CLI " +
			"docs section under content/cli is weighted higher in search) into a " +
			"local cache and lets you search and read it offline. Override the " +
			"source with --repo/--ref/--path or VOLCANO_DOCS_*.\n\n" +
